...
box %>% filter(Thtrs. > 100) %>%
  ggplot(aes(x = Date, y = Total.Gross.adjust, 
             group = interaction(Movie, Distributor))) + geom_line() +
  geom_text(aes(x = Date, y = Max.Gross.adjust, label = Movie), 
            data = box_summary %>% filter(Max.Gross.adjust > 2.5e8))
```
...
box %>% filter(Thtrs. > 100) %>%
  ggplot(aes(x = Date, y = Total.Gross.adjust, 
             group = interaction(Movie, Distributor))) + geom_line() +
  ggrepel::geom_text_repel(aes(x = Date, y = Max.Gross.adjust, label = Movie), 
            data = box_summary %>% filter(Max.Gross.adjust > 2.5e8), colour="grey50")
```
