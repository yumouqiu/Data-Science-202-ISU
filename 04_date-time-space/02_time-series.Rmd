...
## Box office gross (corrected)

```{r, echo = FALSE}
box %>% filter(Thtrs. > 100) %>%
  ggplot(aes(x = Date, y = Total.Gross.adjust, 
             group = interaction(Movie, Distributor))) + geom_line()
```
...
box %>% filter(Thtrs. > 100) %>%
  ggplot(aes(x = Date, y = Total.Gross.adjust, 
             group = interaction(Movie, Distributor))) + geom_line() +
  geom_text(aes(x = Date, y = Total.Gross.adjust, label = Movie), 
            data = box_summary %>% filter(Total.Gross.adjust > 2.5e8))
```
...
box %>% filter(Thtrs. > 100) %>%
  ggplot(aes(x = Date, y = Total.Gross.adjust, 
             group = interaction(Movie, Distributor))) + geom_line() +
  ggrepel::geom_text_repel(aes(x = Date, y = Total.Gross.adjust, label = Movie), 
            data = box_summary %>% filter(Total.Gross.adjust > 2.5e8), colour="grey50")
```
