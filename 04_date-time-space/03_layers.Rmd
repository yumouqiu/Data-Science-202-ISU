...
box %>% filter(Total.Gross.adjust > 1 * 10^8 & Week < 100) %>%
  ggplot(aes(x = Week, y = Total.Gross.adjust, 
        group = interaction(Movie, Distributor))) + 
  geom_line() + 
  geom_text(aes(x = Week, y = Max.Gross.adjust, label = Movie, color = Movie), 
        data = box_summary %>% arrange(desc(Max.Gross.adjust)) %>% head(3))
```
